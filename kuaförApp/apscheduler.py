...
def schedule_task(firma_kod):
    ...
    scheduler.add_job(tamamlanmamis_siparisleri_kontrol_et, 'date', run_date=datetime.now() + timedelta(minutes=1), args=[firma_kod])
    scheduler.add_job(iptal_siparisleri_kontrol_et, 'date', run_date=datetime.now() + timedelta(seconds=1), args=[firma_kod])
